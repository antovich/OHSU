...
MplusLocation <- 'H:/Projects/Y1-Y6 Comorbidity GMM for Sarah/Mplus Scripts/RMLCA/'
...
#Labels for plot below (make sure order aligns with Mplus input)
NewLabs <- c("Anxiety Age 8","Anxiety Age 9","Anxiety Age 10","Anxiety Age 11","Anxiety Age 12","Anxiety Age 13","Anxiety Age 14",
             "CD Age 8","CD Age 9","CD Age 10","CD Age 11","CD Age 12","CD Age 13","CD Age 14",
...
filename = "[[classes]]-class RMCLA.inp";
outputDirectory = "', MplusLocation, '";
[[/init]]

TITLE: RMLCA Comparison
...
SAVEDATA:
FILE IS "[[classes]]-class RMLCA Ouput.csv";
SAVE = CPROB;'
...
MplusOutput <- readModels(str_replace(MplusLocation, "RMLCA/", "RMLCA"))
#####################################################################################################
...
#Grab number of variables from data loading above
VarNum = length(MplusDataByAge)-1
...
ClassMeanData <- MplusData %>%
  select(colnames(MplusDataByAge)[2:(VarNum+1)], C)%>%
  melt(id.vars ="C") %>%
...
ClassPropData <- MplusData %>%
  select(colnames(MplusDataByAge)[2:(1+VarNum)], C)%>%
  melt(id.vars ="C") %>%
