...
setwd('/private/tmp/antovichVolumes/antovich/Projects/Y6 EATQ LPA for Sarah n400/')
...
PlotIndices = c("LogLik","AIC","BIC","SABIC","ICL","Entropy","BLRT_val","BLRT_p", "prob_min","prob_max")
###########################################################################################
...
FullData <- read_csv('/private/tmp/antovichVolumes/antovich/Projects/Y6 EATQ LPA for Sarah n400/Data/Y6 EATQ and Demo data 8-21-19.csv', na = c("", "NA", "-999"))

#Read in Y1 data for Y1 status variable and rename variable for consistency
Y1Data <- read_csv('/private/tmp/antovichVolumes/antovich/Projects/Y1 Factor Scores for Mike n1400/Data/all_data_adhd_ML_project_persistent_11-Sep-2018.csv', na = c("-999","NA",""))%>%
...
DXchange = paste(FullData$Y1_ADHD_STATUS, FullData$Y6_ADHD_STATUS, sep = " to ")
...
EATQ_Cont <-FullData %>%
  filter(FullData[StatusVar] == 1) %>% 
  select(MERGEID, Y6_P_EATQ_ACTIVCONT:Y6_C_EATQ_HIP, -Y6_P_EATQ_EFFCONT, -Y6_P_EATQ_SURGENCY, -Y6_P_EATQ_NEGAFFECT) %>%
  na.omit()
...
adhdPlot <- ADHD_Cast %>%
  ...
  ggtitle(paste(Group, " ", Year," Status ", Reporter, " EATQ LPA ", NumClass, "-Class", sep =""))
