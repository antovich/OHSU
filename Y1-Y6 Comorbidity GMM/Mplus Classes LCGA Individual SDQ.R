...
MplusLocation <- 'H:/Projects/Y1-Y6 Comorbidity GMM for Sarah/Mplus Scripts/LGCA/LGCA SDQ'
...
filename = "[[classes]]-class LGCA SDQTS Clust.inp";
...
CATEGORICAL ARE 
SDQTS', AgeRange[1],'-SDQTS', last(AgeRange),';
...
i s | SDQTS7@0 SDQTS8@1 SDQTS9@2 
SDQTS10@3 SDQTS11@4 SDQTS12@5 
SDQTS13@6 SDQTS14@7
...
SAVEDATA:
FILE IS "[[classes]]-class LGCA SDQTS Clust Ouput.csv";
SAVE = CPROB;'
...
SummariesAll <- NULL
for (i in paste0("X",FirstClass:LastClass,".class.lgca.sdqts.clust.out")) {
...
#####################################################################################################
MplusOutput[[paste0("X",ModelNumber,".class.lgca.sdqts.clust.out")]]$covariance_coverage
MplusOutput[[paste0("X",ModelNumber,".class.lgca.sdqts.clust.out")]]$errors
MplusOutput[[paste0("X",ModelNumber,".class.lgca.sdqts.clust.out")]]$warnings
MplusOutput[[paste0("X",ModelNumber,".class.lgca.sdqts.clust.out")]]$summaries
...
MplusData <- MplusOutput[[paste0("X",ModelNumber,".class.lgca.sdqts.clust.out")]]$savedata
...
library(longCatEDA)
LongObj <- longCat(select(MplusData, 1:VarNum), Labels = c("None","Mild","Clinical"), tLabels = AgeRange)
LongObjSort <- sorter(LongObj, group = MplusData$C, groupLabels = c("High \nDecrease","Moderate \nStable", "Low \nStable"))
longCatPlot(LongObjSort, xlab = "Age", main = VarLab, groupBuffer = 0.2, lcex = 1)
...
for (v in CatVarList) {
p <- ggplot(filter(PropData, variable == v), aes(x = C, y = count, fill = as.factor(value))) +
...
for (v in ContVarList) {
  p <- ggplot(filter(MeanData, variable == v), aes(x = C, y = mean)) +
