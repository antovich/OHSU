...
MplusLocation <- 'H:/Projects/Y1-Y6 Polygenic Scores ADHD Trajectory for Joel/Mplus Scripts/LCGA Aux'
...
MplusData <- OutcomeData %>%
  ...
  select(MERGEID, SCORE, paste0("ADHDRS",YearRange),paste0("KSADHYP",YearRange),paste0("KSADINT",YearRange),paste0("SWAN",YearRange),paste0("CONHYP",YearRange),paste0("CONINT",YearRange))%>%
  mutate(SCORE = (SCORE*10000)-270)
...
filename = "[[classes]]-class LCGA AUX [[Measure#outcome]].inp";
...
USEVARIABLES ARE 
[[Measure#outcome]]', YearRange[1],'-[[Measure#outcome]]', last(YearRange),' SCORE;

!DEFINE # OF CLASSES
CLASSES are c([[classes]]);

AUXILIARY = SCORE(R3STEP);
...
i s ',ifelse(Quad, 'q', ''),' | [[Measure#outcome]]1@0 [[Measure#outcome]]2@1 [[Measure#outcome]]3@2 
[[Measure#outcome]]4@3 [[Measure#outcome]]5@4 [[Measure#outcome]]6@5;
...
SAVEDATA:
FILE IS "[[classes]]-class Symptom LCGA Aux [[Measure#outcome]] Output.csv";
SAVE = CPROB;'
