...
#In this instance, the missing data are handled using multiple imputation
...
MplusLocation <- 'H:/Projects/Y1-Y6 Comorbidity GMM for Sarah/Mplus Scripts/LCGA/Imputation/Data By Age'
...
UseList <- "ANX7 ANX8 ANX9 ANX10 ANX11 ANX12 ANX13 ANX14 CD7 CD8 CD9 CD10 CD11
     CD12 CD13 CD14 MOOD7 MOOD8 MOOD9 MOOD10 MOOD11 MOOD12 MOOD13 MOOD14 ODD7 ODD8
     ODD9 ODD10 ODD11 ODD12 ODD13 ODD14 ADHDSTAT SEX ARSHYP ARSINT KSADINT KSADHYP
     IQ CONEF CONHYP CONINT ACTCONT ACTIVITY AFFIL ANGER FEAR HIP IMPULS INHIBIT SAD
     SHY SOOTHE ASSERT ATTFOCUS LIP PERCEPT DISCOMF OPENNESS FAMID"
...
MissDataset <- DataByAge %>%
  rename(MOOD7 = Age7_DTEAM_MOOD, ANX7 = Age7_DTEAM_ANX, ODD7 = Age7_DTEAM_ODD, CD7 = Age7_DTEAM_CD, 
         ...
         ADHDSTAT = Y1_ADHD_STATUS, ARSHYP = Y1_P_ADHDRS_HYP_TS,	ARSINT = Y1_P_ADHDRS_INT_TS, KSADINT = Y1_KSAD_INTSX,	KSADHYP = Y1_KSAD_HYPSX, IQ = WISCWIAT_FSIQ,
  ...
  dplyr::select(MERGEID, paste0("ANX",AgeRange),paste0("CD",AgeRange),paste0("MOOD",AgeRange),paste0("ODD",AgeRange),
         ADHDSTAT, SEX, ARSHYP,	ARSINT, KSADINT,	KSADHYP, IQ, CONEF,	CONHYP,	CONINT, ACTCONT, ACTIVITY, AFFIL,
         ANGER, FEAR, HIP, IMPULS, INHIBIT, SAD, SHY, SOOTHE, ASSERT, ATTFOCUS, LIP, PERCEPT, DISCOMF, OPENNESS)

#Get column numbers for variables that need coding change for rule outs
firstcol <- which(colnames(MissDataset) == paste0("ANX", AgeRange[1]))
lastcol <- which(colnames(MissDataset) == paste0("ODD", last(AgeRange)))
...
imputerVars <- c(paste0("ANX",AgeRange),paste0("CD",AgeRange),paste0("MOOD",AgeRange),paste0("ODD",AgeRange),
                 "ADHDSTAT", "SEX", "ARSHYP",	"ARSINT", "KSADINT",	"KSADHYP", "IQ", "CONEF",	"CONHYP",	"CONINT", "ACTCONT", "ACTIVITY", "AFFIL",
                 "ANGER", "FEAR", "HIP", "IMPULS", "INHIBIT", "SAD", "SHY", "SOOTHE", "ASSERT", "ATTFOCUS", "LIP", "PERCEPT", "DISCOMF", "OPENNESS")
...
!This is most relevant if covariance (the default) does not converge
MODEL = SEQUENTIAL;
...
filename = "[[classes]]-class LCGA Age Imputation Set 1 [[Measure#outcome]].inp";
...
SAVEDATA:
FILE IS "[[classes]]-class LCGA By Age Imputation [[Measure#outcome]] Output.csv";
SAVE = CPROB;'
...
SValues <- str_match(read_file(paste(MplusLocation, '/',ClassNumber,'-class lcga age imputation set 1 ',tolower(VarName),'.out', sep ="")), 
...
writeLines(paste(
  'TITLE: Dx LCGA Imputation with Age Data
DATA: FILE = 
"', paste(MplusLocation,"\n" ,'missimplist.dat', sep ="/"),'";
TYPE = IMPUTATION;
...
USEVARIABLES ARE 
',VarName, AgeRange[1],'-',VarName, last(AgeRange),';

!DEFINE CATEGORICAL INDICATORS 
!(Must specify categories if not identical)

CATEGORICAL ARE 
',paste0(VarName,AgeRange,'(*)', collapse = " "),';
...
SAVEDATA:
FILE IS "',ClassNumber,'-class LCGA By Age ',VarName,' Output.csv";
SAVE = CPROB;'
  , sep = ""), paste(MplusLocation, MplusMIFile, sep ="/"))

runModels(paste(MplusLocation, MplusMIFile, sep ="/"), replaceOutfile = "modifiedDate")

MplusOutput <- readModels(paste0(MplusLocation, "/missimp2.out"))
...
SummariesAll <- NULL
for (i in paste0("X",FirstClass:LastClass,".class.lcga.age.imputation.set.","1.",tolower(VarName),".out")) {
...
Summaries <- SummariesAll %>%
  ...
  dplyr::select(Classes, AIC, BIC, aBIC, Entropy, VLMR_pVal = T11_VLMR_PValue) %>%
...
MplusData <- MplusOutput[[paste0("X",FirstClass:LastClass,".class.lcga.age.imputation.set.","1.",tolower(VarName),".out")]]$savedata
